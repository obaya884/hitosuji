...
- 挙動を変えない依存追随・ツール整備は `docs/技術改善バックログ.md` に **T-XX** で記録する（[CLAUDE.md](../../../CLAUDE.md) の軸分け）
...
- 挙動を変えない更新なら `docs/技術改善バックログ.md` に完了記録として T-XX を残す
...
- dismiss したものは `docs/技術改善バックログ.md` に T-XX で追跡（根治条件と再確認のトリガーを書く）
...
- **根治できる（合図後の PR）** → コード・ワークフローを直して push。default setup が再スキャンし、指摘が消えれば**アラートは自動クローズ**する（手で閉じない）。挙動を変えない設定修正なら `docs/技術改善バックログ.md` に **T-XX** で記録
...
- dismiss したものは §6 と同様 `docs/技術改善バックログ.md` に **T-XX** で追跡（根治条件と再確認トリガーを書く）
...
- 技術活動の台帳: [docs/技術改善バックログ.md](../../../docs/技術改善バックログ.md)
