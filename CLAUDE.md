...
docs のファイル名は**全体で一意な2桁番号**を持つ（0X=画面定義書内 / 1X=仕様 / 2X=案件 / 3X=検討。新規文書は該当ブロックの次番号を使う）。`log_` / `closed_` / `guide_` の付属ファイルは親の番号を引き継ぐ（例: `log_23_技術改善バックログ.md`）。

未実装要求の案件管理（何を・どの状態で・いつ着手するか）は `docs/案件/22_要件バックログ.md`（プロダクト軸のライブ台帳。決定状態＋着手トリガ）が持つ。挙動レベルの仕様判断は上の各定義書へ先に反映し、バックログはチェックと一行要約に留める。MVP 期のフェーズ運用の記録は `docs/案件/24_実装計画.md`（史実アーカイブ）。
