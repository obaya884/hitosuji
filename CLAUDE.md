...
実運用で得たオーナーの気づきは `docs/案件/ユーザーフィードバック.md` に起票してから対応する。**着手時は `docs/案件/着手手順_ユーザーフィードバック.md` に従い、要求定義書 → 要件定義書 → 画面定義書 の順に上から判定する**（FB は画面の話として書かれるため、画面定義書だけ直して上位文書への反映を忘れやすい。既存の F-XXX / N-XX に収まらない FB は要求定義書に新しいIDを起こす）。
...
- **台帳（FB・技術改善・要件バックログ）の完了・対応済みエントリは、状態更新と同じコミットで同階層の `完了記録_<文書名>.md` へ原文のまま移す**（ライブ台帳には未完了だけを残し、AI が日常的に読むファイルを小さく保つ）
