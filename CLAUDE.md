...
既定は常に単独モード（メインセッションが実装）。**オーナーが並行実行を明示的に指示したとき（例「これとこれを並行で」）だけ**、**ヘッド1＋ワーカーN** の体制に切り替える——AI が自律的に切り替える判断基準は設けない（体制・運用手順は `docs/検討/34_git運用と並行開発体制検討.md` §4〜§7 が正）。要点:
...
**詳細（命名の文字集合・マージキュー・ブランチ削除の作法・並行時の規律）は `docs/検討/34_git運用と並行開発体制検討.md` §2 が正**。常時使う判断は次の2つ:
