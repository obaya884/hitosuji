...
実運用で得たオーナーの気づきは `docs/案件/ユーザーフィードバック.md` に起票してから対応する。**着手時は同ファイルの「着手手順」に従い、要求定義書 → 要件定義書 → 画面定義書 の順に上から判定する**（FB は画面の話として書かれるため、画面定義書だけ直して上位文書への反映を忘れやすい。既存の F-XXX / N-XX に収まらない FB は要求定義書に新しいIDを起こす）。
