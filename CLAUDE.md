...
- 要件・方式・現況: `docs/要件定義書.md`
- データ構造・主要ロジック: `docs/データモデル定義書.md`
- 画面・操作・ショートカット: `docs/画面定義書/*.md`（全画面共通の UI は `00_共通.md`）
- 挙動を変えない技術活動（技術負債の返済・リファクタ・依存追随・CI/ツール整備）: `docs/技術改善バックログ.md`（**T-XX** で管理。上の要求軸とは別軸。挙動が絡むなら要求軸、絡まないならこちら、で振り分ける）

未実装要求の案件管理（何を・どの状態で・いつ着手するか）は `docs/要件バックログ.md`（プロダクト軸のライブ台帳。決定状態＋着手トリガ）が持つ。挙動レベルの仕様判断は上の各定義書へ先に反映し、バックログはチェックと一行要約に留める。MVP 期のフェーズ運用の記録は `docs/実装計画.md`（史実アーカイブ）。
...
- **リファクタ棚卸しをするとき** — ①オーナーが「リファクタしたい／コードが大きくなった」と言ったとき、②機能をいくつか重ねた節目、③新しい負債の兆候に気づいたとき、のいずれかで `refactor-scout` を起動し、返ってきた T-XX 案をオーナー確認のうえメインが `docs/技術改善バックログ.md` へ記入する。定常フローには載せず、**契機が来たときに回す**（毎コミットでは起動しない）
