...
並行させたいタスクが複数たまったときは、**ヘッド1＋ワーカーN** の体制に切り替える（体制・運用手順は `docs/検討/34_git運用と並行開発体制検討.md` §4〜§7 が正）。要点:
...
- **機能が完成したら** — 仕様が絡む一連の実装を終えた時点で `spec-reviewer`（docs 整合）と `test-reviewer`（テスト観点の網羅・正しさ・カバレッジ）を並列起動する
...
1. **サブエージェントによるレビュー**: `verifier`（lint/build/型/テスト等の機械的検証）・`code-quality-reviewer`（可読性・重複・命名・簡潔さの品質レビュー）・`spec-reviewer`（docs/ 仕様書群との整合）・`test-reviewer`（テスト観点の網羅・正しさ・カバレッジ）を並列起動し、指摘があればメインセッションで対処する
...
**詳細は `docs/検討/34_git運用と並行開発体制検討.md` §2 が正**。要点:
...
- **ブランチ命名**: `<タスクID>-<slug>`（小文字・`a-z 0-9 -` のみ・スラッシュなし。例 `fb-54-section-jump`）。タスクID は T-XX / FB-XX / F-XXX / N-XX、ID の無い作業は先に台帳へ起票する。**1ブランチ＝1PR、使い回さない。マージ後はローカル・リモートともブランチ削除**
- **並行時の規律**: スキーマ変更を含むブランチは常に最大1本。マージは1本ずつ（マージ → 残りを rebase → CI 再走 → 次）
- **自動マージ・CI 必須チェック化（ブランチ保護）は設けない**。`main` への push・PR マージは常にオーナーの合図。マージ方式は squash（main のログを線形に保つ）
