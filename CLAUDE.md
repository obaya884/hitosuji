...
**プロダクト挙動を変えない技術活動**（技術負債の返済・リファクタ・依存追随・CI/ツール整備など）は上の要求軸ではなく `docs/技術改善計画.md` で **T-XX** として管理する。挙動が絡むなら要求軸、絡まないなら技術改善計画、で振り分ける。
...
1. **サブエージェントによるレビュー**: `verifier`（lint/build/型/テスト等の機械的検証）と `spec-reviewer`（docs/ 仕様書群との整合）を並列起動し、指摘があればメインセッションで対処する
