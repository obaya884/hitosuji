...
- 要求: `docs/仕様/要求定義書.md`（F-XXX / N-XX の一覧）
- 要件・方式・現況: `docs/仕様/要件定義書.md`
- データ構造・主要ロジック: `docs/仕様/データモデル定義書.md`
- 画面・操作・ショートカット: `docs/仕様/画面定義書/*.md`（全画面共通の UI は `00_共通.md`）
- 挙動を変えない技術活動（技術負債の返済・リファクタ・依存追随・CI/ツール整備）: `docs/案件/技術改善バックログ.md`（**T-XX** で管理。上の要求軸とは別軸。挙動が絡むなら要求軸、絡まないならこちら、で振り分ける）

未実装要求の案件管理（何を・どの状態で・いつ着手するか）は `docs/案件/要件バックログ.md`（プロダクト軸のライブ台帳。決定状態＋着手トリガ）が持つ。挙動レベルの仕様判断は上の各定義書へ先に反映し、バックログはチェックと一行要約に留める。MVP 期のフェーズ運用の記録は `docs/案件/実装計画.md`（史実アーカイブ）。

実運用で得たオーナーの気づきは `docs/案件/ユーザーフィードバック.md` に起票してから対応する。**着手時は `docs/案件/guide_ユーザーフィードバック.md` に従い、要求定義書 → 要件定義書 → 画面定義書 の順に上から判定する**（FB は画面の話として書かれるため、画面定義書だけ直して上位文書への反映を忘れやすい。既存の F-XXX / N-XX に収まらない FB は要求定義書に新しいIDを起こす）。
...
- **判断の経緯（「なぜそうしたか」）は本文に書かず、各文書と同階層の `log_<文書名>.md` へ日付順に積む**（本文側の「決定ログ」節は §番号を保った参照スタブ。画面定義書は `docs/仕様/画面定義書/log_NN_*.md`）。本文は現在の仕様だけを書く。将来再検討しうる前提だけ本文に1行残してよい。決定ログを持たない文書は最初のエントリが出た時点で同階層に `log_<文書名>.md` を作る
...
クリーンアーキテクチャ＋関数型DDD。**コード配置・依存方向・テスト戦略の契約は `docs/仕様/アーキテクチャ定義書.md` が正**。要点:
...
- **リファクタ棚卸しをするとき** — ①オーナーが「リファクタしたい／コードが大きくなった」と言ったとき、②機能をいくつか重ねた節目、③新しい負債の兆候に気づいたとき、のいずれかで `refactor-scout` を起動し、返ってきた T-XX 案をオーナー確認のうえメインが `docs/案件/技術改善バックログ.md` へ記入する。定常フローには載せず、**契機が来たときに回す**（毎コミットでは起動しない）
...
- **本番の実データを書かない** — タスク名・ルーチン名などオーナーの生活が読み取れる内容は、docs の例示にも使わない（`docs/案件/ユーザーフィードバック.md` は特に混入しやすい）
...
- **具体的な手順（リモート推奨／ローカル）は [README](./README.md)「スキーマ更新（本番マイグレーション）」が正**。方式・設計の背景は [docs/検討/スキーマ更新パイプライン検討.md](./docs/検討/スキーマ更新パイプライン検討.md)（T-22）
