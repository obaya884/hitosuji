...
- 段階1（案A）実装: `.github/workflows/db-migrate.yml` を新設。`workflow_dispatch`（手動ボタン）＋ `production` Environment の承認ゲートで `npm run db:migrate` を本番へ適用する。デプロイは従来どおり別途手動のため順序リスクは現行と同じ。**GitHub 側の Environment 作成・`MIGRATE_DATABASE_URL`（unpooled）シークレット・required reviewers の設定はオーナーが手動で行う**（未設定では動かない。検討ドキュメント §4・§7）
