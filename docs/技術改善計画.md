...
## T-22 本番スキーマ更新をリモートで実行するパイプライン（案A・段階1 実装済み）

- 種別: ツール整備（当初は調査）/ 優先度: 中 / 状態: 進行中（案A・段階1 実装。案B・Cは未着手）
- 背景: 本番マイグレーションは手元での手動実行（[CLAUDE.md](../CLAUDE.md)「本番マイグレーションの手順」）。これをリモート（GitHub Actions）で完結させ、接続情報を Environment シークレットに置きたい
- 方式: **A（実行のリモート化・`workflow_dispatch`）→ B（マージ契機で migrate→deploy を自動化し順序を機械保証）→ C（expand/contract で順序依存を除去）** の段階で高度化する。難所は migrate 実行そのものではなく (1) Vercel 自動デプロイとの順序保証、(2) public リポジトリでのシークレット露出、の2点。**詳細は [スキーマ更新パイプライン検討.md](./スキーマ更新パイプライン検討.md) が正**
- 段階1（案A）実装: `.github/workflows/db-migrate.yml` を新設。`workflow_dispatch`（手動ボタン）＋ migrate 専用 `db-migrate` Environment の承認ゲートで `npm run db:migrate` を本番へ適用する（Vercel の "Production" 環境とは分離）。デプロイは従来どおり別途手動のため順序リスクは現行と同じ。**GitHub 側の Environment 作成・`MIGRATE_DATABASE_URL`（unpooled）シークレット・required reviewers の設定はオーナーが手動で行う**（未設定では動かない。検討ドキュメント §4・§7）
- 残り: 案B（Vercel 自動デプロイ無効化＋ migrate 成功時デプロイ発火）・案C は着手条件を満たしたら
