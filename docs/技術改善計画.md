...
## T-25 データバックアップの自動化（案A 採用・実装未着手）

- 種別: ツール整備（当初は調査）/ 優先度: 中 / 状態: 未着手（方式決定済み）
...
- 方式: 難所は「取得の実行」ではなく **public リポジトリで実データを含むダンプをどこへどう置くか**。これを (1) 暗号化 (2) 保存先 に分解し、**`age` 非対称暗号（公開鍵はコミット可・復号鍵は CI に載せない）で秘匿を保存先に依存させない**設計に。保存先は A（プライベート GitHub リポジトリ）→ B（R2/B2・lifecycle）→ C（S3/GCS・OIDC）と投資が上がるが、**オーナーは両クラウド未保有・単一ユーザー・極小データ**につき **A を採用**、B・C は将来の選択肢。**詳細は [データバックアップ自動化検討.md](./データバックアップ自動化検討.md) が正**
- 実装: 未着手（本 T は方式決定まで）。着手する場合は非公開の保存先リポジトリ＋push 用 PAT・age 鍵ペア・`BACKUP_DATABASE_URL`（unpooled）シークレット・`.github/workflows/db-backup.yml`（`schedule`＋`workflow_dispatch`／`pg_dump`→age→push／prune／失敗通知）を用意する（検討ドキュメント §5・§7）。定期実行のため**承認ゲートは付けず**「非 PR イベント限定＋最小権限＋失敗通知」で担保する
