...
## T-22 本番スキーマ更新をリモートで実行するワークフロー（案A 採用）

- 種別: ツール整備（当初は調査）/ 優先度: 中 / 状態: 進行中（案A ＋ 運用明瞭化＝注意コメント・README を実装済み。オーナーの GitHub Environment 設定とマージで完了）
- 背景: 本番マイグレーションは手元での手動実行（[CLAUDE.md](../CLAUDE.md)「本番マイグレーションの手順」）。これをリモート（GitHub Actions）で完結させ、接続情報を Environment シークレットに置きたい
- 方式: 実現方式は A（実行のリモート化・`workflow_dispatch`）→ B（マージ契機の自動 migrate→deploy）→ C（expand/contract）と高度化しうるが、**個人開発の規模では A で必要十分**と判断し **A を採用。B・C は過剰につき見送り**（将来の選択肢として記録）。難所は migrate 実行そのものではなく (1) Vercel 自動デプロイとの順序保証、(2) public リポジトリでのシークレット露出、の2点。**詳細は [スキーマ更新パイプライン検討.md](./スキーマ更新パイプライン検討.md) が正**
- 実装（案A）: `.github/workflows/db-migrate.yml` を新設。`workflow_dispatch`（手動ボタン）＋ migrate 専用 `db-migrate` Environment の承認ゲートで `npm run db:migrate` を本番へ適用する（Vercel の "Production" 環境とは分離）。新規マイグレーションを持つ **PR ブランチから実行 → 成功後にマージ（＝デプロイ）** の順で回す。**GitHub 側の Environment 作成・`MIGRATE_DATABASE_URL`（unpooled）シークレット・required reviewers の設定はオーナーが手動で行う**（未設定では動かない。検討ドキュメント §3・§4）
- 運用の明瞭化: スキーマ更新のある PR に migrate 実行を促す注意コメント（T-21 の label workflow を拡張）と、README への運用記載を足す
...
- 関連: 本書 T-22（スキーマ更新のリモート化・同型の作業場）/ [README](../README.md)「バックアップ（N-06）」/ [データモデル定義書](./データモデル定義書.md) §6
