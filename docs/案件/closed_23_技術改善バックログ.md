...
- 前提の訂正: 当初の「**E2E を入れない限り数字は埋まらない**」は誤りだった。Client Component（22ファイル・約3,600行＝`.tsx` の大半）はコンポーネントテスト（jsdom）で測れる。E2E が要るのは async Server Component（13ファイル・約450行）だけ。この切り分けは [T-39](./23_技術改善バックログ.md#t-39-コンポーネントテストjsdomの導入) で実装し、4段の呼び名と境界は[アーキテクチャ定義書](../仕様/15_アーキテクチャ定義書.md) §8 に記録
...
- 決めたこと: ①public リポジトリで数値を公開してよい ②単一のバッジ・全体値のみは採らない ③外部サービス（Codecov 等）は使わない ④計測対象は変えず [T-33](./23_技術改善バックログ.md#t-33-カバレッジ計測に-tsx-を含めるかの検討)（`.tsx` を含めるか）と独立を保つ。理由は[決定ログ](./log_23_技術改善バックログ.md)
