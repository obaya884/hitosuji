...
- 関連文書: [アーキテクチャ定義書](../仕様/アーキテクチャ定義書.md)（設計・テスト戦略）/ [要件バックログ](./要件バックログ.md)（プロダクト軸の台帳）
...
- 優先度: `高` / `中` / `様子見`
- 状態: `未着手` / `進行中` / `完了` / `保留`
- 判断の経緯は本文に書かず、[log_技術改善バックログ.md](./log_技術改善バックログ.md) へ日付順に積む（CLAUDE.md「書き方の規約」に従う）
...
- 背景: テストピラミッドの頂点。現状は手動の実運用確認で代替している（[アーキテクチャ定義書](../仕様/アーキテクチャ定義書.md) §8）。プロダクト要求（F-XXX / N-XX）には乗らない技術活動のため、実装計画 Phase 3 に置かれたまま行き場がなかったのを本書へ移した
...
- 背景: 現状は単一ワークツリー・単一セッションで作業しており、別タスクが並行すると変更が交錯する。実際、別セッションで進行中だった作業（`docs/案件/ユーザーフィードバック.md` ほかの WIP）を中断・除外して本作業を進める必要が生じた。複数の作業を安全に並行させる仕組みが無い
...
- 関連: 本書 T-22（スキーマ更新のリモート化・同型の作業場）/ [README](../../README.md)「バックアップ（N-06）」/ [データモデル定義書](../仕様/データモデル定義書.md) §6
...
- 方式（着手時に詰める）: (1) リスク別ルーティング（docs/低リスク＝main 直コミット／コード・挙動＝ブランチ＋CI＋プレビュー→合図でマージ／スキーマ＝T-22 のマイグレーション先行）、(2) push＝デプロイのオーナー合図規律の粒度、(3) 並行化の運用（ブランチ命名・マージ順序・スキーマ変更は常に最大1本・CI を統合ゲートに）。まとめ先は CLAUDE.md、長くなるなら専用検討ドキュメント。設計は [AI並行開発体制検討.md](../検討/AI並行開発体制検討.md)（T-06）と統合する
- 着手条件: 並行開発を実際に始める前、または現行の混在運用が実害になったとき
- 暫定運用: 確定までは軽微・docs 変更は `main` 直コミットで進める（オーナーの従来運用に合わせる）
- 関連: 本書 T-06（AI並行開発体制）/ T-22（本番スキーマ更新パイプライン）/ [CLAUDE.md](../../CLAUDE.md)「このリポジトリは public」「本番マイグレーションの手順」
...
技術活動（T-XX）に関する判断は[log_技術改善バックログ.md](./log_技術改善バックログ.md)に日付順で積む。
