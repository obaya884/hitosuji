...
| comment | text | NULL可 | 一言コメント（F-206, Phase 3） |
...
展開されたタスクを削除したとき、その日にそのルーチンを再展開しないための記録（F-304 のスキップ。2026-07-20 追加）。
...
- 過去日のスキップ記録は残るが害がないため削除しない（Phase 2 のレビューで「その日やらなかった」根拠として使える可能性がある）
...
2. 再開タスク T' を INSERT: name/mode/project は T と同値、section_id と sort_order は上記の配置位置に従う、split_parent_id = T.id
   - estimate_minutes = **T の見積もりが未設定（0分）なら 0（未設定のまま引き継ぐ）**、設定済みなら max(T.estimate − T実績, 1)（2026-07-19 オーナー判断: 未設定タスクに1分の見積もりが勝手に付くのを避ける）
...
DBには保存しない導出値。**表示日のリストに属するタスクのみを対象**にクライアントで計算する（前日以前から放置されている実行中タスクは含めない。打刻忘れであり実作業ではないため。2026-07-19 オーナー判断。放置分は画面定義書01 §8 の警告バナーで別途知らせる）:
...
### 4.4 未実行タスクの自動セクション移動（F-113, Phase 2）
...
- **対象行**: `task_date` = 今日 の行のうち、
  - §4.2-a（開始時）: **開始したタスク自身**の1行のみ。`started_at` を書き込むのと同一トランザクションで移す（`section_id IS NULL` の未分類からも移る）
  - §4.2-b（繰り下げ）: `started_at IS NULL` かつ `section_id IS NOT NULL` の行
- **採番**: 移動先の直前・直後の `sort_order` の中間値を取る間隔採番（§3.5）。複数件をまとめて繰り下げる場合は、元の相対順序を保ったまま連続した位置へ割り当てる。中間値が取れなくなったら当日分を1000刻みで振り直す（§3.5 の再採番と同じ）
...
- 週次で `pg_dump`（カスタム形式）を取得する。手順とコマンドは README を正とする（`npm run db:backup` / `npm run db:restore`）
- リストアは `pg_restore` で任意の Postgres へ可能。**2026-07-20 に本番ダンプからの復元を実演済み**（行数・内容ハッシュの一致と、復元DBでのアプリ動作を確認）
- 自動化（GitHub Actions の cron。Neon の接続文字列は Actions secrets に保管）は実運用を開始してから検討する
