...
- **日界（1日の開始時刻）の将来導入に備える**: `task_date` は保存された論理日付であり、打刻時刻（timestamptz）から導出しない。このため日界を後から導入しても既存データは影響を受けず、変更点は「今日」の解決（アプリ起動時の表示日・ルーチン展開対象日・終了予定計算の対象日）とセクション表示順の回転（§3.1）に閉じる。現状は 0:00 固定（設定可能化は要求 F-116。Phase 2）
