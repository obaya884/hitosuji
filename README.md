...
- 仕様: [要求定義書](./docs/仕様/要求定義書.md) / [要件定義書](./docs/仕様/要件定義書.md) / [データモデル定義書](./docs/仕様/データモデル定義書.md) / [画面定義書](./docs/仕様/画面定義書/)
- 案件の管理: [要件バックログ](./docs/案件/要件バックログ.md)（プロダクト軸）/ [技術改善バックログ](./docs/案件/技術改善バックログ.md)（技術軸）/ [ユーザーフィードバック管理簿](./docs/案件/ユーザーフィードバック.md)（FB 軸）/ [実装計画](./docs/案件/実装計画.md)（MVP 期アーカイブ）
...
テスト（アーキテクチャ・テスト戦略は [docs/仕様/アーキテクチャ定義書.md](./docs/仕様/アーキテクチャ定義書.md)）:
...
PR と main への push では GitHub Actions（`.github/workflows/ci.yml`）が lint・build・test を自動実行する（統合テストは Postgres サービスコンテナを建てる）。Dependabot の依存更新PRもここで検証される。技術改善・負債返済などの活動は [技術改善バックログ](./docs/案件/技術改善バックログ.md) で管理する。
...
新規マイグレーション（`src/infrastructure/db/migrations/*.sql`）を含む PR には、CI が自動で**「スキーマ更新」ラベル**と**実行を促す注意コメント**を付ける（[技術改善バックログ完了記録](./docs/案件/closed_技術改善バックログ.md) T-21）。適用は次のどちらか。方式・設計の背景は [docs/検討/スキーマ更新パイプライン検討.md](./docs/検討/スキーマ更新パイプライン検討.md)（T-22。案A を採用し、自動化の案B・Cは個人開発の規模では過剰につき見送り）。
...
GitHub Actions cron による自動化は暗号化方式を確定済み（`age` 非対称暗号）・保存先は2案併記で保留（R2/B2 or S3/GCS+OIDC）。方式・設計は [docs/検討/データバックアップ自動化検討.md](./docs/検討/データバックアップ自動化検討.md)（T-25）が正。実装は運用の負担を見てから着手する。
