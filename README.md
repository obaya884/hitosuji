...
新規マイグレーション（`src/infrastructure/db/migrations/*.sql`）を含む PR には、CI が自動で**「スキーマ更新」ラベル**と**実行を促す注意コメント**を付ける（[技術改善バックログ](./docs/案件/技術改善バックログ.md) T-21）。適用は次のどちらか。方式・設計の背景は [docs/検討/スキーマ更新パイプライン検討.md](./docs/検討/スキーマ更新パイプライン検討.md)（T-22。案A を採用し、自動化の案B・Cは個人開発の規模では過剰につき見送り）。
