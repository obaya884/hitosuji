 ...
 * 「実行中タスクの終了 → 複製タスクの生成 → 再開タスクの生成」を1トランザクションで行う。
...
export type DuplicateAndStartCommand = Readonly<{
  ...
  /** 挿入位置に中間値が無かったときの、移動先セクションの振り直し（データモデル定義書 §3.5） */
  renumber: Renumber;
}>;
...
export type TaskRepository = Readonly<{
   ...
   * あわせて routine_id・bundle_id・section_id を落とす:
   * - routine_id: 移動先の日にはその日のぶんが改めて展開される（紐付けたまま移すと展開の一意制約に衝突する。同書 §3.5）
   * - bundle_id: 同じ理由（同書 §4.8。付けたまま移すと同じバンドルに同名が2件並ぶ）
   * - section_id: セクションは元の日の時間帯への割り当てなので持ち越さず、移動先の日の未分類で改めて
   *   トリアージする（同書 §3.5）。**sortOrder は呼び出し側が移動先の未分類で採番して渡す**
