...
export function moveTaskByStep(
  ...
  const positionInGroup = group.findIndex((t) => t.id === taskId);
  const nextIndex = positionInGroup + step;

  if (nextIndex >= 0 && nextIndex < group.length) {
    return reorderTask(tasks, taskId, {
  ...
  const index = step === 1 ? 0 : sorted(tasks, neighbor).length;
  return reorderTask(tasks, taskId, { sectionId: neighbor, index });
}
