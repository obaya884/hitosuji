...
import { actualMinutes, carryOverDays, elapsedMinutes } from "./task";
import { task } from "./testing/task";

const startedAt = new Date("2026-07-26T08:00:00Z");

describe("carryOverDays（F-122 / データモデル定義書 §3.5: task_date − initial_task_date）", () => {
  it("先送りされた日数を返す", () => {
    const t = task({ id: 1, initialTaskDate: "2026-09-18", taskDate: "2026-09-21" });
    expect(carryOverDays(t)).toBe(3);
  });
...
describe("actualMinutes（データモデル定義書 §3.5: 実績 = ended_at − started_at）", () => {
  it("完了タスクの実績を分で返す", () => {
    const t = task({ id: 1, startedAt, endedAt: new Date("2026-07-26T08:18:00Z") });
    expect(actualMinutes(t)).toBe(18);
  });

  it("1分未満は常に0分（切り捨て。画面定義書01 §3.3: 表示は 0:00）", () => {
    const short = task({ id: 1, startedAt, endedAt: new Date("2026-07-26T08:00:20Z") });
    expect(actualMinutes(short)).toBe(0);

    // 45秒は四捨五入だと1分になってしまうため、切り捨てであることを固定する
    const almostAMinute = task({ id: 1, startedAt, endedAt: new Date("2026-07-26T08:00:45Z") });
    expect(actualMinutes(almostAMinute)).toBe(0);
  });

  it("端数は切り捨てる（1分59秒は1分）", () => {
    const t = task({ id: 1, startedAt, endedAt: new Date("2026-07-26T08:01:59Z") });
...
describe("elapsedMinutes（F-205: 実行中タスクの経過時間）", () => {
  it("実行中タスクの経過を現在時刻から求める", () => {
    ...
    expect(elapsedMinutes(t, new Date("2026-07-26T08:12:00Z"))).toBe(12);
  });

  it("経過も端数は切り捨てる（開始直後は0分）", () => {
    const t = task({ id: 1, startedAt });
    expect(elapsedMinutes(t, new Date("2026-07-26T08:00:45Z"))).toBe(0);
  });

  it("現在時刻が開始時刻より前でも負値ではなく0を返す（FB-28: クライアント時計のズレ対策）", () => {
    const t = task({ id: 1, startedAt });
    expect(elapsedMinutes(t, new Date("2026-07-26T07:59:30Z"))).toBe(0);
  });

  it("未実行・完了タスクは経過を持たない", () => {
    expect(elapsedMinutes(task({ id: 1 }), new Date())).toBeNull();
    expect(
      elapsedMinutes(task({ id: 1, startedAt, endedAt: new Date("2026-07-26T08:30:00Z") }), new Date())
    ).toBeNull();
  });
});
