 ...
 * （種別切り替え時に再計算せず使えるようにするため。不要な項目は validateRoutineInput が null に落とす）
 */
 ...
 */
export function routineEstimateFromTask(task: Task): Result<number, RoutineFromTaskError> {
...
/** タスク＋入力 → ルーチンの入力値（未検証の RoutineInput）（画面定義書01 §4.1） */
export function routineInputFromTask(
  task: Task,
  choice: RoutineFromTaskChoice
): Result<RoutineInput, RoutineFromTaskError> {
  ...
  return ok({
    ...
    scheduledStartTime: choice.scheduledStartTime,
    modeId: task.modeId,
