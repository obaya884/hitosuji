// モード管理の Server Action の**失敗経路**を実DBから直接叩く（T-112）。
...
const FAILURE_ARGS: { [K in keyof typeof actions]: Parameters<(typeof actions)[K]> } = {
  createModeAction: [{ name: "", color: COLOR_BY_NAME["赤"] }],
  updateModeAction: [MISSING_ID, { name: "仕事", color: COLOR_BY_NAME["赤"] }],
  setModeArchivedAction: [MISSING_ID, true],
  deleteModeAction: [MISSING_ID],
};

describe("モード管理の全アクションが失敗を値として返す（形の規約を全数で確かめる）", () => {
