...
import { inlineInputClass, type InlineInputType } from "./editable-cell";

/** 新規追加行で入力する項目。書き手と読み手の綴りをコンパイラに突き合わせさせるため型で閉じる */
export type NewRowField = "name" | "startTime";
...
type Props = Readonly<{
   ...
   */
  onSave: (fieldValue: (field: NewRowField) => string) => void;
  onCancel: () => void;
  /**
   * 値の入力セル（`<td>` 群）。入力欄は `NewRowInput` に `onKeyDown` を渡して作る
   * （保存中は `undefined` が渡る＝キー操作を受け付けない）
   */
  renderCells: (onKeyDown: KeyboardEventHandler<HTMLInputElement> | undefined) => ReactNode;
}>;

export function NewRow({ isPending, onSave, onCancel, renderCells }: Props) {
  /**
...
export function NewRowInput({
  ...
  /** `NewRow` の `onSave` が値を読むときの名前 */
  field: NewRowField;
  placeholder?: string;
  type?: InlineInputType;
  autoFocus?: boolean;
  /** `NewRow` から受け取る（保存中は `undefined`） */
