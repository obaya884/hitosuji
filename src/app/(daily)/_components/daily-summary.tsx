...
export function DailySummary({ groups, now, isToday, dayStartMinutes }: Props) {
  ...
  return (
    ...
    <div className="flex items-center gap-4 font-mono text-main tabular-nums">
      {/* 1日全体の進捗は表示日によらず出す（過去日の振り返りでも見たいため。§3.1） */}
      <span className="flex items-center gap-2">
        <TaskProgress tasks={tasks} textSize="text-main" />
      </span>
      {/* 終了予定・現在・残作業は当日表示時のみ（§3.1） */}
      {isToday && (
        <>
          <span>
            <span className="font-sans text-sub text-ink-muted">終了予定 </span>
            <span className={overMidnight ? "font-medium text-danger" : "font-medium"}>
              {formatProjectedEnd(end, now, APP_TIME_ZONE, dayStartMinutes)}
            </span>
          </span>
          <span>
            <span className="font-sans text-sub text-ink-muted">現在 </span>
            {formatClock(now)}
          </span>
          {/* 「残」だけだと空き時間と誤読されるため「残作業」とする（§3.1 / FB-22） */}
          <span>
            <span className="font-sans text-sub text-ink-muted">残作業 </span>
