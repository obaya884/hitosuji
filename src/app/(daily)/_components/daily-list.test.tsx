...
describe("DailyList（画面定義書01 §3.2/§3.3: 1タスク=1行のテーブル型リスト）", () => {
  ...
  describe("セクション見出し行（§3.2）", () => {
    ...
    it("時間合計は完了は実績・未完了は見積もりを合算する（分母はセクション枠の長さ）", () => {
      ...
      const heading = headingOf("朝");
      expect(within(heading).queryByText("0:48")).not.toBeNull();
      expect(within(heading).queryByText("/3:00")).not.toBeNull();
    });

    it("時間合計が0のときは `--:--`（00_共通 §2.4）", () => {
      renderList({ groups: [morning([task({ id: 1, name: "朝食", estimateMinutes: 0 })])] });
    ...
    it("0件のグループは見出しだけを置き、進捗・時間合計・残り時間を出さない（FB-25/FB-26）", () => {
      ...
      const heading = headingOf("朝");
      expect(within(heading).queryByText("06:00–09:00")).not.toBeNull();
      expect(heading.textContent).not.toContain("合計");
      expect(heading.textContent).not.toContain("0/0");
    });
    ...
    it("残り時間は（セクション終了 − 現在）− 未完了見積もり（F-110）", () => {
      ...
      const heading = headingOf("朝");
      expect(heading.textContent).toContain("残り");
      expect(within(heading).queryByText("+1:30")).not.toBeNull();
    });
  ...
  describe("タスク行の状態別表示（状態は打刻から導出する）", () => {
    it("未実行は開始ボタン（押したときの動作を示す。F-201）", () => {
      renderList({ groups: [morning([task({ id: 1, name: "日次プラン" })])] });
  ...
  describe("未設定の表記（00_共通 §2.4: 空欄にしない）", () => {
    it("プロジェクト・モードの未設定は薄色の `-`", () => {
      renderList({ groups: [morning([task({ id: 1, name: "日次プラン" })])] });

      const { project, mode } = cellsOf(rowOf("日次プラン"));
      for (const cell of [project, mode]) {
    ...
    it("見積もり設定済みは H:MM を通常色で出す", () => {
      renderList({ groups: [morning([task({ id: 1, name: "日次プラン", estimateMinutes: 15 })])] });
  ...
  describe("行選択（§5）", () => {
    it("行のクリックで選択する（マウスとキーボードは等価）", () => {
      const { onSelect } = renderList({ groups: [morning([task({ id: 7, name: "朝食" })])] });
    ...
    it("固定領域の高さぶん余白を取り、追従した行が裏に隠れないようにする（§2 / §5）", () => {
      renderList({ stickyHeight: 96, groups: [morning([task({ id: 1, name: "朝食" })])] });
  ...
  describe("打刻ボタン（F-201）", () => {
    it("押すと打刻し、その行を選択する", () => {
      const { onPunch, onSelect } = renderList({
        groups: [morning([task({ id: 5, name: "日次プラン" })])],
      });
  ...
  describe("インライン編集（00_共通 §2.3）", () => {
    it("タスク名のクリックで編集を始める", () => {
      const { onBeginEdit } = renderList({ groups: [morning([task({ id: 1, name: "朝食" })])] });
    ...
    it("編集中は現在の値を初期値に入れる", () => {
      renderList({
        editing: { taskId: 1, field: "name" },
        groups: [morning([task({ id: 1, name: "朝食" })])],
      });
    ...
    it("入力欄から離れたら確定する（§2.3「確定」）", () => {
      const { onRename, onEndEdit } = renderList({
        editing: { taskId: 1, field: "name" },
        groups: [morning([task({ id: 1, name: "朝食" })])],
      });
    ...
    it("編集は行内の1セルだけに効く（他のセルは表示のまま残る）", () => {
      renderList({
        editing: { taskId: 1, field: "name" },
        groups: [morning([task({ id: 1, name: "朝食", estimateMinutes: 20 })])],
      });
    ...
    it("見積もりは分の整数で編集する（F-103）", () => {
      const { onEstimate, onEndEdit } = renderList({
        editing: { taskId: 1, field: "estimate" },
        groups: [morning([task({ id: 1, name: "朝食", estimateMinutes: 20 })])],
      });
    ...
    it("見積もり未設定（0分）の編集は空欄から始める（`0` を消す手間を作らない）", () => {
      renderList({
        editing: { taskId: 1, field: "estimate" },
        groups: [morning([task({ id: 1, name: "朝食", estimateMinutes: 0 })])],
      });
    ...
    describe("打刻時刻の修正（F-203）", () => {
      ...
      it("編集を始めたときは既存の値を全選択する（打ち直しが前提のため。FB-23）", () => {
        renderList({ editing: { taskId: 1, field: "startedAt" }, groups: [morning([completed])] });
      ...
      it("区切り文字なしの入力をそのまま渡す（整形は受け取り側が担う）", () => {
        const { onEditPunch, onEndEdit } = renderList({
          editing: { taskId: 1, field: "startedAt" },
          groups: [morning([completed])],
        });
      ...
      it("終了時刻の編集は終了時刻を初期値にする（開始と取り違えない）", () => {
        renderList({ editing: { taskId: 1, field: "endedAt" }, groups: [morning([completed])] });
  ...
  describe("モード・プロジェクトの選択（O-5）", () => {
    it("モードの候補は「モードなし」＋有効モードのみ（アーカイブ済みは出さない。画面定義書03 §4）", () => {
      const { onAssign } = renderList({
        editing: { taskId: 1, field: "mode" },
        groups: [morning([task({ id: 1, name: "朝食", modeId: 1 })])],
      });
    ...
    it("プロジェクトの候補は「プロジェクトなし」＋有効プロジェクトのみ", () => {
      const { onAssign } = renderList({
        editing: { taskId: 1, field: "project" },
        groups: [morning([task({ id: 1, name: "朝食" })])],
      });
  ...
  describe("行メニュー（O-7 / O-8 / O-12）", () => {
    ...
    it("ルーチン由来のタスクはルーチン化を非活性で見せる（§4.1 / FB-30）", () => {
      renderList({ groups: [morning([task({ id: 1, name: "朝食", routineId: 9 })])] });
      openMenu(rowOf("朝食"));
    ...
    it("複製を選ぶと duplicate で通知する（O-11 / F-111）", () => {
      const { onOperate } = renderList({ groups: [morning([task({ id: 1, name: "朝食" })])] });
      openMenu(rowOf("朝食"));
    ...
    it("先送りを選ぶと postpone で通知する（O-7 / F-107。行メニューからのみ実行できる）", () => {
      const { onOperate } = renderList({ groups: [morning([task({ id: 1, name: "日次プラン" })])] });
      openMenu(rowOf("日次プラン"));
    ...
    it("ルーチン化ポップオーバーの作成で通知し、編集を閉じる（§4.1）", () => {
      const { onRoutinize, onEndEdit } = renderList({
        editing: { taskId: 1, field: "routinize" },
        groups: [morning([task({ id: 1, name: "朝食", estimateMinutes: 20, sectionId: 100 })])],
      });
