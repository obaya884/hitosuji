...
// 投入されるモードの色（画面定義書03 §3.2 のプリセットのどれか）はマスタリポジトリの統合テストが
// 3件とも固定している（`drizzle-mode-repository.int.test.ts`）。ここで見るのは投入の作法——何を入れ、
// **2回目に何をしないか**。何度流しても足さない・壊さないが冪等スクリプトを名乗る条件（T-60）
describe("seedMasters（データモデル定義書 §5: 空テーブルにのみ投入する冪等スクリプト）", () => {
  it("空のテーブルへデータモデル定義書 §5 のとおり投入する", async () => {
